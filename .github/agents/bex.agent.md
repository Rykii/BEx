...
| **碳金融** | 碳配额、碳信用、碳回购、碳远期 |
| **收益互换** | 境外期货、境内期货、境外债券、境内债券、商品 |
| **利率互换** | SOFR等 |
| **场外期权** | 境外收益率、指数ETF、商品 |
| **其他衍生品** | CDS、CMO、MBS、CCS |
| **收益凭证** | 自研指数 |
| **债券及ETF** | 境外债、境内债、债券回购（正/逆/买断式）、场外ETF |
| **对冲交易** | 债券、国债期货、利率、权益ETF、商品 |

#### 文档定稿前的完整性交叉扫描（2026-05-20 新增）

> **背景**：2026-05-20 发现 source_registry 对外汇核心信源 CFETS 仅收录 2 条，大量产品指南页未归档。根因是 registy 仅覆盖"已归档文件"，未扫描报告正文中的隐性引用。
...
文档定稿前，BEx §7 执行：
    │
    ▼
1. 提取报告正文中所有URL（含附录链接表、脚注引用、正文内链）
    │
    ▼
2. 逐一匹配 source_registry：URL 是否在对应业务大类的 sources[] 中？
    │
    ├── 是 → ✓ 已登记
    └── 否 → ⚠ 写入 outstanding_sources[]
    │
    ▼
3. 按业务大类统计覆盖率：
   覆盖率 = sources[] 数量 / (sources[] 数量 + outstanding_sources[] 数量)
   ⚠ 覆盖率 < 80% → 告警，提示 Recon 补充搜索
    │
    ▼
4. 特别检查：outstanding 中是否存在 L2 交易所/基础设施信源？
   是 → P1 优先级，应尽快归档（因产品定义/交易规则层最易遗漏）
```

**禁止放行条件：**
- 文档引用了某官方域名但 registry 对应大类下该域名的条目为空 → 必须标记为 outstanding
- 覆盖率 < 50% → 文档状态降级为 `draft`，不得标记为 `final`
...
- [ ] 报告正文中是否存在 source_registry 未覆盖的 URL？（§7 交叉扫描）
- [ ] 对应业务大类的 L2 交易所/基础设施信源覆盖率是否 ≥ 80%？（若 <80% 不得定稿）
